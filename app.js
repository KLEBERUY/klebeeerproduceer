...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup scroll detection for mobile menu profile visibility
...
function setupScrollDetection() {
    ...
    if (isMobile) {
        ...
        window.addEventListener('scroll', updateProfileMenuVisibility);
    }
}
...
function updateProfileMenuVisibility() {
    ...
    const isAtTop = scrollTop < 100; // Consider "top" as first 100px
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.nav-link').forEach(link => {
        console.log('Setting up nav link:', link);
...
function loadBeatsFromFirestore() {
    ...
    db.collection('beats').get()
        .then(snapshot => {
            console.log('Beats snapshot size:', snapshot.size);
            beatsGrid.innerHTML = '';
            ...
            if (snapshot.empty) {
                ...
                beatsGrid.innerHTML = `
                    <div class="empty-state">
                        ...
                        <p>No hay beats disponibles todavía</p>
                        <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
                            <button class="btn btn-primary" onclick="switchSection('upload')">
                                <i class="fas fa-plus"></i> Subir el primer beat
                            </button>
                            <button class="btn btn-secondary" onclick="createSampleBeats()">
                                <i class="fas fa-magic"></i> Crear beats de ejemplo
                            </button>
                        </div>
                    </div>
                `;
        ...
        .catch(error => {
            ...
            beatsGrid.innerHTML = '<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><p>Error al cargar los beats</p><button class="btn btn-secondary" onclick="loadBeatsFromFirestore()"><i class="fas fa-refresh"></i> Reintentar</button></div>';
        });
}
...
// Create sample beats for testing (if database is empty)
function createSampleBeats() {
    console.log('Creating sample beats for testing...');
    ...
    const sampleBeats = [
        {
            ...
            coverUrl: "https://picsum.photos/seed/trap1/300/200.jpg",
        ...
        },
        {
            ...
            coverUrl: "https://picsum.photos/seed/hiphop2/300/200.jpg",
            uploadedBy: "demo-user"
        },
        {
            name: "R&B Melody",
            ...
            coverUrl: "https://picsum.photos/seed/rnb3/300/200.jpg",
            uploadedBy: "demo-user"
        }
    ];
    
    sampleBeats.forEach((beat, index) => {
        db.collection('beats').add({
            ...beat,
            uploadedAt: firebase.firestore.FieldValue.serverTimestamp()
        }).then(() => {
            console.log(`Sample beat ${index + 1} created`);
            if (index === sampleBeats.length - 1) {
                loadBeatsFromFirestore(); // Reload after creating all samples
            }
        }).catch(error => {
            console.error('Error creating sample beat:', error);
        });
    });
}
...
function hideNotification() {
    ...
    // Also remove inline styles that might interfere with the transition
    notification.style.opacity = '0';
    notification.style.transform = 'translateX(20px)';
    
    // After transition completes, reset position
    setTimeout(() => {
...
function switchSection(sectionId) {
    ...
    if (targetSection) {
        ...
        // Handle search bar visibility on mobile
        handleSearchBarVisibility(sectionId);
    } else {
        console.error('Section not found:', sectionId);
    }
}

function handleSearchBarVisibility(sectionId) {
...
function handleProfileAvatarClick() {
    ...
    // Options for avatar click functionality
    if (currentUser) {
        // Option 1: Upload new avatar
        showNotification('Función de cambio de avatar próximamente', 'info');
        
        // Option 2: Could open file picker to upload new avatar
        ...
        // Option 3: Could show profile options menu
        // showProfileOptionsMenu();
    } else {
        showNotification('Debes iniciar sesión para cambiar tu avatar', 'error');
    }
}
...
window.handleProfileAvatarClick = handleProfileAvatarClick;
window.updateProfileMenuVisibility = updateProfileMenuVisibility;
window.backToCart = backToCart;

// Specific function for back to cart button
function backToCart() {
